                ...
                if (args[1].equalsIgnoreCase("drop_boost_amount")) {
                    if (args.length == 2) {
                        ...
                        try {
                            ...
                            gck.getConfig().set("config.dropBoost.amount", value);

                            gck.saveConfig();

                            if (value > 0) {
                                player.sendMessage(ChatColor.WHITE + "현재 " + ChatColor.YELLOW + "열쇠 갯수 드랍 부스트" + ChatColor.WHITE + "는 " + ChatColor.GREEN + "+" + ChatColor.GREEN + gck.getConfig().getInt("config.dropBoost.amount") + ChatColor.GREEN + "개" + ChatColor.WHITE + " 입니다.");
                            } else {
                    ...
                    if (args.length == 2) {
                        ...
                        try {
                            ...
                            if (value < -100) {
                                ...
                                gck.getConfig().set("config.dropBoost.chance", value);

                                gck.saveConfig();


                                if (value >= 0) {
                                    player.sendMessage(ChatColor.WHITE + "현재 " + ChatColor.YELLOW + "열쇠 드랍 확률 부스트" + ChatColor.WHITE + "는 " + ChatColor.GREEN + "+" + ChatColor.GREEN + gck.getConfig().getInt("config.dropBoost.chance") + ChatColor.GREEN + "%" + ChatColor.WHITE + " 입니다.");
                                } else {
                                    player.sendMessage(ChatColor.WHITE + "현재 " + ChatColor.YELLOW + "열쇠 드랍 확률 부스트" + ChatColor.WHITE + "는 " + ChatColor.RED + gck.getConfig().getInt("config.dropBoost.chance") + ChatColor.RED + "%" + ChatColor.WHITE + " 입니다.");
                                }
                                player.sendMessage(ChatColor.YELLOW + "입력한 값과 일치하는지 확인해 주세요.");
                            }
                        } catch (NumberFormatException e) {
